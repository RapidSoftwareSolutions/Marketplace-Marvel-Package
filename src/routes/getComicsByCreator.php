<?php
$app->post('/api/Marvel/getComicsByCreator', function ($request, $response, $args) {
    $settings = $this->settings;

    //checking properly formed json
    $checkRequest = $this->validation;
    $validateRes = $checkRequest->validate($request, ['publicKey', 'privateKey', 'creatorId']);
    ...
    $query_str = $settings['api_url'] . "v1/public/creators/" . $post_data['args']['creatorId'] . '/comics';
    $body = array();
    ...
    if (isset($post_data['args']['dateRange']) && strlen($post_data['args']['dateRange']) > 0) {
        $body['dateRange'] = $post_data['args']['dateRange'];
    }
    ...
    if (isset($post_data['args']['characters']) && strlen($post_data['args']['characters']) > 0) {
        $body['characters'] = $post_data['args']['characters'];
    }
